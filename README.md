...
$$\begin{equation}
    \frac{d}{dt} \hat{\rho}(t) = -\frac{i}{\hslash} [\hat{H}(t),\hat{\rho}(t)] + \sum_{j=1}^N \gamma_j \left(\hat{L}_j \hat{\rho}(t) \hat{L}_j^\dagger - \frac{1}{2} \big\{\hat{L}_j^\dagger \hat{L}_j, \hat{\rho}(t)\big\}\right), \quad N \leq d = \dim(\mathbf{Hbold}_S),
\end{equation}$$
